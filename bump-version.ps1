...
# 브랜치 가드 — master 외 브랜치에서 릴리스 범프/태그를 만들지 않는다(불변 규칙: master만 릴리스 대상).
# release.yml은 태그 push에 트리거되므로, 비-master 커밋에 v* 태그가 붙으면 미검토 브랜치가 발행될 수 있다.
$branch = git -C $root rev-parse --abbrev-ref HEAD
if ($LASTEXITCODE -ne 0) { throw "git rev-parse failed (repo root: $root)" }
$branch = "$branch".Trim()
if ($branch -ne 'master') {
    throw "refusing to bump on branch '$branch' - release policy is master only. checkout master first."
}
...
try {
    # pathspec를 명시 — 사전에 스테이징된 무관 변경이 버전 범프 커밋에 휩쓸리지 않게 한다.
    git add $csproj $installerCsproj
    if ($LASTEXITCODE -ne 0) { throw 'git add failed' }
    git commit -m "chore: bump version to $new" -- $csproj $installerCsproj
    if ($LASTEXITCODE -ne 0) { throw 'git commit failed' }
    git tag "v$new"
    if ($LASTEXITCODE -ne 0) { throw "git tag failed (tag v$new already exists?)" }
    if (-not $NoPush) {
    ...
    # 부분 실패 시 재실행이 버전을 또 범프하지 않도록 수동 복구 절차를 안내(자동 롤백은 안전상 하지 않음).
    Write-Host ''
    Write-Host "!! bump failed: $($_.Exception.Message)" -ForegroundColor Red
    Write-Host "!! recovery: 스크립트를 무조건 재실행하지 말 것(버전 결번 위험). 실패 단계만 수동 재시도:" -ForegroundColor Yellow
    Write-Host "     git -C `"$root`" status" -ForegroundColor Yellow
    Write-Host "     # 커밋됐고 태그/푸시만 실패면:  git -C `"$root`" push origin master; git -C `"$root`" push origin v$new" -ForegroundColor Yellow
    Write-Host "     # 커밋 전이면 csproj 되돌리기:   git -C `"$root`" checkout -- `"$csproj`" `"$installerCsproj`"" -ForegroundColor Yellow
    throw
}
finally {
    Pop-Location
