...
`AgentUsageMonitor-Installer.exe`(권장)는 진행 상황 창을 표시하므로 이 문제가 거의 없습니다 —
"보안 검사" 단계에서 기다리라는 안내가 보이면 정상 진행 중입니다.
아래는 **구방식 `AgentUsageMonitor-win-Setup.exe`를 직접 실행한 경우**의 안내입니다.

Setup.exe는 별도 마법사 창 없이 짧은 로고 화면(splash)만 잠깐 보이고 자동으로 진행됩니다.
...
> **주요(메이저) 버전 업그레이드**(예: 1.x → 2.x)는 설치 방식이 달라 인앱 업데이트가 지원되지
> 않습니다. 이 경우 앱이 설치 버튼 대신 릴리스 페이지 링크를 안내합니다 — 새 인스톨러로
> 설치하면 되고, 기존 데이터·설정은 그대로 유지됩니다.
...
dotnet test tests/ClaudeUsageMonitor.Installer.Tests
